...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // OPTIMIZED: Use getCountFromServer for stats (no document reads)
      const [
        bookingsCountSnap,
        customReqCountSnap,
        techCountSnap,
      ...
      // OPTIMIZED: Fetch only first 5 items for display (not entire collections)
      const [
        pendingBookingsSnap,
        recentBookingsSnap,
        recentTechSnap,
      ...
      // Calculate completed bookings from recent bookings (not all bookings)
      const completedCount = recentBookingsSnap.docs.filter(doc => doc.data().status === 'completed').length;

      setStats({
        totalBookings: bookingsCountSnap.data().count,
        pendingBookings: pendingBookingsSnap.size,
        customRequests: customReqCountSnap.data().count,
        pendingCustomRequests: customReqCountSnap.data().count,
        techApplications: techAppsCountSnap.data().count,
        activeTechnicians: techCountSnap.data().count,
      ...
      setPendingBookings(pendingBookingsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
