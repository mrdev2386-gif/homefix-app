...
// ============================================================================
// ❌ UNUSED SCREEN - DO NOT USE
// ============================================================================
// This file is NOT imported or used anywhere in the codebase.
// The active services screen is: lib/features/technician/services/services_screen.dart
// 
// REASON FOR KEEPING:
// - Kept as reference implementation for potential future optimization
// - Contains useful patterns for service list rendering
// 
// STATUS: DISABLED - Not imported in any navigation or routing
// LAST VERIFIED: Production audit - no usages found
// 
// DO NOT USE THIS FILE. Use services_screen.dart instead.
// ============================================================================

// Status mapping constants - avoid repeated computations
...
class _ServicesListStreamState extends State<_ServicesListStream> {
  static const int _pageSize = 20; // Load 20 items per page instead of 100
  DocumentSnapshot? _lastDocument;
  bool _hasMore = true;
  final List<QueryDocumentSnapshot> _allServices = [];
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          addAutomaticKeepAlives: true, // ✅ FIXED: Keep widgets alive during scroll
...
class _ServiceCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isActive = FirestoreSafeParser.toSafeBool(service['isActive'], fallback: true);
    final district = FirestoreSafeParser.toSafeString(service['district'], fallback: 'N/A');
    final rating = FirestoreSafeParser.toSafeDouble(service['averageRating']);
    final reviews = FirestoreSafeParser.toSafeInt(service['totalReviews']);
    
    debugPrint('[SERVICE CARD] ID: $serviceId, Status: ${service['status']}');
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _ServiceCardActions(serviceId: serviceId, service: service),
        ],
      ),
    );
  }
  ...
  Widget _buildServiceImage(String imageUrl) {
    final safeImageUrl = FirestoreSafeParser.toSafeString(service['imageUrl']);
    ...
    // ✅ OPTIMIZED: Use simple Image.network with timeout and error handling
    return ClipRRect(
      borderRadius: BorderRadius.circular(12),
      child: Image.network(
        safeImageUrl,
  ...
  Widget _buildServiceInfo(bool isActive, String district, double rating, int reviews) {
    ...
    final status = FirestoreSafeParser.toSafeString(service['status'], fallback: 'pending');
    
    // ✅ OPTIMIZED: Use pre-computed status config
    final config = _statusConfig[status] ?? _statusConfig['pending']!;
    final displayStatus = config['label'] as String;
    final statusColor = config['color'] as Color;
    final statusBgColor = config['bgColor'] as Color;
    ...
    return Column(
      ...
      children: [
        Text(
          FirestoreSafeParser.toSafeString(service['name'], fallback: 'Unnamed Service'),
          style: GoogleFonts.plusJakartaSans(
            fontSize: 15,
            fontWeight: FontWeight.bold,
            color: const Color(0xFF0F172A),
        ...
        Wrap(
          ...
          children: [
            ...
            if (FirestoreSafeParser.toSafeBool(service['nightService']?['enabled']))
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(
            ...
            if (FirestoreSafeParser.toSafeBool(service['urgentBooking']?['enabled']))
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(
...
class _ServiceCardActions extends StatefulWidget {
  final String serviceId;
  final Map<String, dynamic> service;
  ...
  @override
  State<_ServiceCardActions> createState() => _ServiceCardActionsState();
}
