Fix the Firebase callable function authentication issue preventing `updateUserProfile` from saving state and district.

Context:
The Flutter customer app logs show:
...
The function `updateUserProfile` is being called but Firebase Auth context is not properly attached, causing the callable function to reject the request.

Goal:
Ensure Firebase Auth token is refreshed and attached before calling the Cloud Function.

Tasks:

1. Open the Flutter file where `updateUserProfile` is called (profile save action).

2. Before calling the Firebase Function, force refresh the Firebase Auth ID token.

Add this code before the function call:

await FirebaseAuth.instance.currentUser?.getIdToken(true);

3. Ensure the callable function is invoked like this:
...
4. Ensure the user is authenticated before calling the function.
...
5. Do NOT change the function name, payload structure, or Firestore schema.

6. Keep the existing backend logic intact.

Expected Result:

When the user selects state and district and presses save:

* Firebase Auth token is refreshed
* Cloud Function receives authenticated request
* updateUserProfile executes successfully
* Firestore document customers/{uid} updates:
