...
exports.verifyTechnicianNormalization = exports.normalizeTechnicianData = void 0;
 ...
 */
exports.normalizeTechnicianData = functions.region('asia-south1').https.onCall(async (data, context) => {
    // Admin only function
    if (!context.auth || !context.auth.token.admin) {
        throw new functions.https.HttpsError('permission-denied', 'Only admins can run data normalization');
    }
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            try {
                ...
                if (normalizedSteps.personalDetails === true)
                    completedSteps++;
                if (normalizedSteps.serviceCategories === true)
                    completedSteps++;
                if (normalizedSteps.portfolio === true)
                    completedSteps++;
                if (normalizedSteps.verification === true)
                    completedSteps++;
        ...
        throw new functions.https.HttpsError('internal', `Normalization failed: ${error}`);
    }
});
/**
 ...
 */
exports.verifyTechnicianNormalization = functions.region('asia-south1').https.onCall(async (data, context) => {
    // Admin only function
    if (!context.auth || !context.auth.token.admin) {
        throw new functions.https.HttpsError('permission-denied', 'Only admins can run verification');
    }
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            let completedSteps = 0;
            if (steps.personalDetails === true)
                completedSteps++;
            if (steps.serviceCategories === true)
                completedSteps++;
            if (steps.portfolio === true)
                completedSteps++;
            if (steps.verification === true)
                completedSteps++;
        ...
        throw new functions.https.HttpsError('internal', `Verification failed: ${error}`);
    }
});
//# sourceMappingURL=technician_normalization.js.map