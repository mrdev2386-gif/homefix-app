...
exports.manageUser = exports.blockUser = exports.updateUser = exports.getUserById = exports.getUsers = void 0;
 ...
 */
exports.getUsers = functions.region('asia-south1').https.onCall((0, security_1.secureCallable)(async (data, context) => {
    try {
        await (0, utils_1.assertAdmin)(context);
        const { limit = 10, offset = 0, role, status, search } = data;
        let query = config_1.db.collection('users');
        if (role) {
 ...
 */
exports.getUserById = functions.region('asia-south1').https.onCall((0, security_1.secureCallable)(async (data, context) => {
    try {
        await (0, utils_1.assertAdmin)(context);
        const { userId } = data;
        if (!userId)
        ...
        const walletDoc = await config_1.db.collection('wallets').doc(userId).get();
        const walletBalance = walletDoc.exists ? (walletDoc.data()?.balance || 0) : 0;
        // Fetch booking history (last 5)
        const bookingsSnap = await config_1.db.collection('bookings')
 ...
 */
exports.updateUser = functions.region('asia-south1').https.onCall((0, security_1.secureCallable)(async (data, context) => {
    try {
        await (0, utils_1.assertAdmin)(context);
        const { userId, updates } = data;
        if (!userId || !updates)
            throw new functions.https.HttpsError('invalid-argument', 'Missing userId or updates');
        ...
        await (0, utils_1.logAdminAction)(context.auth.uid, 'update_user', userId, { updates: cleanUpdates });
        return { success: true };
    }
    catch (error) {
 ...
 */
exports.blockUser = functions.region('asia-south1').https.onCall((0, security_1.secureCallable)(async (data, context) => {
    try {
        await (0, utils_1.assertAdmin)(context);
        ...
        await (0, utils_1.logAdminAction)(context.auth.uid, block ? 'block_user' : 'unblock_user', userId);
        return { success: true };
    }
    catch (error) {
...
exports.manageUser = functions.region('asia-south1').https.onCall((0, security_1.secureCallable)(async (data, context) => {
    try {
        await (0, utils_1.assertAdmin)(context);
        ...
        const userRoleDoc = await config_1.db.collection('users').doc(userId).get();
        ...
        const ref = config_1.db.collection(collection).doc(userId);
        ...
        if (action === 'block' || action === 'unblock') {
            ...
            if (isBlocked && reason) {
                updates.suspensionReason = (0, security_1.sanitize)(reason);
            ...
            await config_1.db.collection('users').doc(userId).update({ isBlocked });
        }
        else if (action === 'make_test') {
            updates.isTestUser = true;
        }
        await ref.update(updates);
        await (0, utils_1.logAdminAction)(context.auth.uid, `user_${action}`, userId, { type, reason: (0, security_1.sanitize)(reason) });
