...
exports.getEligibleTechnicians = exports.respondToBooking = exports.onBookingCreated = void 0;
...
// --- MATCHING ENGINE ---
async function findMatchingTechnicians(subServiceId, location) {
    ...
    for (const doc of snapshot.docs) {
        ...
        if (!tech.skills?.[serviceId]?.subServiceIds?.includes(subServiceId))
            continue;
        // 4. Filter by Radius
        if (!tech.coordinates)
            continue;
        ...
        const distance = (0, geoutils_1.calculateDistance)(techLoc, jobLoc);
...
// --- TRIGGERS ---
exports.onBookingCreated = functions.firestore.document('bookings/{bookingId}')
    ...
    for (const tech of matches) {
        ...
        await (0, notifications_1.sendPushNotification)(tech.id, 'technicians', {
...
// --- TECH RESPONSE ---
exports.respondToBooking = functions.region('asia-south1').https.onCall(async (data, context) => {
    ...
    return db.runTransaction(async (transaction) => {
        ...
        if (action === 'accept') {
            ...
            if (!techDoc.exists)
                throw new functions.https.HttpsError('not-found', 'Technician profile missing');
            const techData = techDoc.data();
            if (techData.status !== 'active')
                throw new functions.https.HttpsError('permission-denied', 'Technician blocked or inactive');
            // 2. Update Booking
 ...
 */
exports.getEligibleTechnicians = functions.region('asia-south1').https.onCall(async (data, context) => {
    // 1. Security Check
    if (!context.auth) {
