...
exports.razorpayBankWebhook = exports.verifyTechnicianBankAccountSecure = void 0;
...
function generateIdempotencyKey(uid, accountNumber) {
 ...
 */
exports.verifyTechnicianBankAccountSecure = functions.region('asia-south1').https.onCall(async (data, context) => {
    ...
    if (!accountHolderName || !accountNumber || !ifscCode) {
        throw new functions.https.HttpsError('invalid-argument', 'All bank details are required');
    }
    // Validate IFSC format (11 characters: 4 letters + 0 + 6 alphanumeric)
    if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifscCode.toUpperCase())) {
        throw new functions.https.HttpsError('invalid-argument', 'Invalid IFSC code format');
    }
    ...
    if (techData.verificationLock === true) {
        throw new functions.https.HttpsError('failed-precondition', 'Verification already in progress. Please wait.');
    }
    // ============================================================================
    ...
    if (lastAttempt && !force) {
        ...
        if (timeSinceLastAttempt < ATTEMPT_WINDOW_MS && attempts >= MAX_ATTEMPTS) {
            throw new functions.https.HttpsError('resource-exhausted', `Too many verification attempts. Please try again after 1 hour.`);
        }
    ...
    try {
        ...
        if (!razorpay) {
            throw new functions.https.HttpsError('failed-precondition', 'Razorpay instance is null');
        }
        // ============================================================================
        ...
        if (!contactId) {
            ...
            try {
                ...
                throw new functions.https.HttpsError('internal', `Failed to create Razorpay contact: ${contactError.message}`);
            }
        }
        else {
        ...
        try {
            ...
            throw new functions.https.HttpsError('internal', `Failed to create fund account: ${fundError.message}`);
        }
 ...
 */
exports.razorpayBankWebhook = functions.https.onRequest(async (req, res) => {
    res.status(200).send({ message: 'Webhook deprecated. Using Fund Account Validation instead.' });
});
//# sourceMappingURL=bank_verification.js.map