...
exports.checkKycStatus = exports.evaluateTechnicianKyc = void 0;
const functions = __importStar(require("firebase-functions"));
const admin = __importStar(require("firebase-admin"));
const https = __importStar(require("firebase-functions/v1/https"));
 ...
 */
exports.evaluateTechnicianKyc = functions.region('asia-south1').https.onCall(async (data, context) => {
    try {
        // 1. Authentication check
        if (!context.auth) {
            throw new https.HttpsError('unauthenticated', 'User must be authenticated to evaluate KYC');
        }
        ...
        const coreComplete = kycChecklist.fullName &&
            kycChecklist.phone &&
            kycChecklist.city &&
        ...
        console.log(`[KYC_EVAL] KYC Complete for ${uid}: ${isKycComplete} (coreComplete=${coreComplete}, bankReady=${kycChecklist.bankDetails})`);
        ...
        throw new https.HttpsError('internal', `Failed to evaluate KYC: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
/**
 ...
 */
exports.checkKycStatus = functions.region('asia-south1').https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
        ...
        throw new https.HttpsError('internal', `Failed to check KYC: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
});
