...
exports.saveTechnicianStepData = exports.updateTechnicianStatus = exports.updateTechnicianProfile = exports.submitTechnicianKyc = exports.saveTechnicianServices = exports.saveTechnicianDocuments = exports.saveTechnicianBasicDetails = exports.createTechnicianProfile = void 0;
 ...
 */
exports.createTechnicianProfile = functions.region('asia-south1').https.onCall(async (data, context) => {
...
exports.saveTechnicianBasicDetails = functions.region('asia-south1').https.onCall(async (data, context) => {
    ...
    const sanitizedFullName = (0, security_1.sanitizeString)(fullName || '', 100);
    const sanitizedEmail = (0, security_1.sanitizeEmail)(email || '');
    const sanitizedDistrict = (0, security_1.sanitizeString)(district || '', 50);
    // Validation
    ...
    if (!techDoc.exists) {
        throw new functions.https.HttpsError('not-found', 'Technician profile not found. Please start onboarding again.');
    }
 ...
 */
exports.saveTechnicianDocuments = functions.region('asia-south1').https.onCall(async (data, context) => {
    (0, security_1.assertAuthenticated)(context);
    const uid = context.auth.uid;
    const { aadhaarNumber, aadhaarFrontUrl, aadhaarBackUrl, profilePhotoUrl, documentType } = data;
    // SECURITY FIX: Sanitize Aadhaar
    const sanitizedAadhaar = (0, security_1.sanitizeAadhaar)(aadhaarNumber || '');
    // Validate required fields
    if (!sanitizedAadhaar || sanitizedAadhaar.length !== 12) {
        throw new functions.https.HttpsError('invalid-argument', 'Valid 12-digit Aadhaar number is required');
    }
    if (!aadhaarFrontUrl || !profilePhotoUrl) {
        throw new functions.https.HttpsError('invalid-argument', 'Document images and profile photo are required');
    }
    // Get current technician to verify onboarding state
    const techDoc = await db.collection('technicians').doc(uid).get();
    if (!techDoc.exists) {
 ...
 */
exports.saveTechnicianServices = functions.region('asia-south1').https.onCall(async (data, context) => {
    (0, security_1.assertAuthenticated)(context);
    const uid = context.auth.uid;
    const { categoryId, categoryName, skills } = data;
    // Validation
    if (!categoryId || !categoryName) {
        throw new functions.https.HttpsError('invalid-argument', 'Category selection is required');
    }
    if (!skills || skills.length === 0) {
        throw new functions.https.HttpsError('invalid-argument', 'At least one skill is required');
    }
    // Verify category exists
    const categoryDoc = await db.collection('categories').doc(categoryId).get();
    if (!categoryDoc.exists) {
        throw new functions.https.HttpsError('not-found', 'Invalid category selected');
    }
    // Get current technician to verify onboarding state
    const techDoc = await db.collection('technicians').doc(uid).get();
    if (!techDoc.exists) {
 ...
 */
exports.submitTechnicianKyc = functions.region('asia-south1').https.onCall(async (data, context) => {
    (0, security_1.assertAuthenticated)(context);
    const uid = context.auth.uid;
    ...
    if (currentStep !== 'review') {
        throw new functions.https.HttpsError('failed-precondition', 'Please complete all onboarding steps before submitting');
    }
    // Validate required fields
    const requiredFields = ['fullName', 'phone', 'aadhaarNumber', 'profilePhotoUrl', 'primaryCategoryId'];
    for (const field of requiredFields) {
        if (!techData?.[field]) {
            throw new functions.https.HttpsError('failed-precondition', `Missing required field: ${field}. Please complete all steps.`);
        }
    }
    ...
    if (!techData?.experienceDescription || techData.experienceDescription.length < 20) {
        throw new functions.https.HttpsError('failed-precondition', 'Experience description must be at least 20 characters');
    }
    if (!techData?.workPreference) {
        throw new functions.https.HttpsError('failed-precondition', 'Work preference is required');
    }
 ...
 */
exports.updateTechnicianProfile = functions.region('asia-south1').https.onCall(async (data, context) => {
    (0, security_1.assertAuthenticated)(context);
    const uid = context.auth.uid;
    const { updates } = data;
 ...
 */
exports.updateTechnicianStatus = functions.region('asia-south1').https.onCall(async (data, context) => {
    (0, security_1.assertAuthenticated)(context);
    const uid = context.auth.uid;
    const { isOnline } = data;
    if (typeof isOnline !== 'boolean') {
        throw new functions.https.HttpsError('invalid-argument', 'isOnline must be a boolean');
    }
    ...
    if (isOnline && (!techData?.isApproved || !techData?.isKycComplete)) {
        throw new functions.https.HttpsError('failed-precondition', 'You must complete onboarding and receive approval before going online');
    }
    // Check for suspended/blocked status
    if (techData?.status === 'suspended' || techData?.status === 'blocked') {
        throw new functions.https.HttpsError('permission-denied', 'Your account is suspended or blocked');
    }
    await db.collection('technicians').doc(uid).update({
        isOnline: isOnline,
 ...
 */
exports.saveTechnicianStepData = functions.region('asia-south1').https.onCall(async (data, context) => {
    (0, security_1.assertAuthenticated)(context);
    const uid = context.auth.uid;
