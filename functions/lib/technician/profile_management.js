...
exports.adminUpdateDocumentStatus = exports.adminUpdateBankStatus = exports.reuploadVerificationDocument = exports.updateTechnicianBankDetails = exports.updateTechnicianPersonalDetails = void 0;
const functions = __importStar(require("firebase-functions"));
const admin = __importStar(require("firebase-admin"));
const db = admin.firestore();
exports.updateTechnicianPersonalDetails = functions.region('asia-south1').https.onCall(async (data, context) => {
...
exports.updateTechnicianBankDetails = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
    const uid = context.auth.uid;
...
exports.reuploadVerificationDocument = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
    const uid = context.auth.uid;
...
exports.adminUpdateBankStatus = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
...
exports.adminUpdateDocumentStatus = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
    const adminDoc = await db.collection('admins').doc(context.auth.uid).get();
...
//# sourceMappingURL=profile_management.js.map