...
exports.monitorBookingHealth = exports.technicianRespondBookingRateLimited = exports.sendBookingNotification = exports.validateWalletIntegrity = exports.updateBookingStatusNew = exports.technicianRespondBooking = void 0;
...
async function storeIdempotency(key, result) {
...
// ============================================
exports.technicianRespondBooking = functions.region('asia-south1').https.onCall(async (data, context) => {
    // Auth check
    if (!context.auth) {
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (booking.status !== 'technician_pending') {
            throw new functions.https.HttpsError('failed-precondition', `Booking already ${booking.status}`);
        }
        // FIX 3: Check if already accepted by another device
        if (booking.acceptedAt) {
            throw new functions.https.HttpsError('already-exists', 'ALREADY_ACCEPTED', { code: 'ALREADY_ACCEPTED' });
        }
        if (action === 'accept') {
            ...
            if (!isAvailable) {
                throw new functions.https.HttpsError('failed-precondition', 'Outside working hours');
            }
            // Use status history tracker for atomic update
            (0, status_history_tracker_1.updateBookingStatus)(transaction, bookingRef, 'awaiting_payment', booking, {
            ...
            // Use status history tracker for atomic update
            (0, status_history_tracker_1.updateBookingStatus)(transaction, bookingRef, 'rejected', booking, {
...
// ============================================
exports.updateBookingStatusNew = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (!allowedNext.includes(status)) {
            throw new functions.https.HttpsError('failed-precondition', `Cannot transition from ${currentStatus} to ${status}`);
        }
        // Update booking
        (0, status_history_tracker_1.updateBookingStatus)(transaction, bookingRef, status, booking, {
...
// ============================================
exports.validateWalletIntegrity = functions.pubsub
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (doc.exists) {
            ...
            if (data.actionCount >= 5) {
                ...
                throw new functions.https.HttpsError('resource-exhausted', 'Too many requests. Please wait 10 seconds.');
            }
...
exports.technicianRespondBookingRateLimited = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
    }
...
// ============================================
exports.monitorBookingHealth = functions.pubsub
...
*/
//# sourceMappingURL=booking_actions_hardened.js.map