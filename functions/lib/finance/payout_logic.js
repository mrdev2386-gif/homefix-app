...
exports.settleTechnicianBalance = exports.razorpayPayoutWebhook = exports.triggerTechnicianPayout = void 0;
...
exports.triggerTechnicianPayout = functions.region('asia-south1').https.onCall(async (data, context) => {
    ...
    const payoutId = config_1.db.collection('payouts').doc().id;
    try {
        await config_1.db.collection('payouts').doc(payoutId).set({
            id: payoutId,
            technicianId,
            amount,
            status: 'initiated',
        ...
        await config_1.db.runTransaction(async (t) => {
            t.update(config_1.db.collection('payouts').doc(payoutId), {
        ...
        await (0, utils_1.logAdminAction)(context.auth.uid, 'payout_initiated', technicianId, { amount, payoutId });
        return { success: true, payoutId, rzpPayoutId: payout.id };
    }
    catch (error) {
        console.error(`${LOG_PREFIX} Payout Error:`, error);
        await config_1.db.collection('payouts').doc(payoutId).update({
            status: 'failed',
            error: error.message,
            updatedAt: admin.firestore.FieldValue.serverTimestamp()
        });
 ...
 */
exports.razorpayPayoutWebhook = functions.https.onRequest(async (req, res) => {
    if (req.method !== 'POST') {
    ...
    try {
        ...
        // FIX 3A: Use technician_wallets (single source of truth)
        const walletRef = config_1.db.collection('technician_wallets').doc(techId);
 ...
 */
exports.settleTechnicianBalance = functions.region('asia-south1').https.onCall(async (data, context) => {
    await (0, utils_1.assertAdmin)(context);
    ...
    if (pending <= 0)
        return { success: true, message: 'No pending balance' };
    await config_1.db.runTransaction(async (t) => {
        t.update(walletRef, {
...
async function initiateRefund(bookingId) {
    ...
    const payments = await config_1.db.collection('payments')
    ...
    try {
        ...
        await notify.notifyCustomerBookingCancelled(paymentData.userId, bookingId, `A refund of ₹${paymentData.amount} has been initiated for your booking.`);
    }
    catch (e) {
        console.error(`${LOG_PREFIX} Razorpay Refund Error:`, e);
    }
}
//# sourceMappingURL=payout_logic.js.map