...
exports.generateTechnicianWalletQR = exports.generateBookingQR = exports.getTransactionHistory = exports.getPayoutHistory = exports.requestWithdrawal = void 0;
...
function generateWithdrawalIdempotencyKey(technicianId, amount, timestamp) {
 ...
 * 5. Log transaction
 */
exports.requestWithdrawal = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (tech.bankVerified !== true || tech.bankVerificationStatus !== 'verified') {
        ...
        throw new functions.https.HttpsError('failed-precondition', 'Please verify your bank account before requesting withdrawal. Go to Profile > Bank Details to verify.');
    }
    if (!tech.fundAccountId) {
        console.warn(`${LOG_PREFIX} Fund account missing`);
        throw new functions.https.HttpsError('failed-precondition', 'Bank account verification incomplete. Please re-verify your bank details.');
    }
    ...
    const todayWithdrawals = await config_1.db.collection('technician_wallets')
        .doc(technicianId)
    ...
    const existingPayout = await config_1.db.collection('payouts')
    ...
    const payoutRef = config_1.db.collection('payouts').doc(payoutId);
    try {
...
exports.getPayoutHistory = functions.region('asia-south1').https.onCall(async (data, context) => {
    ...
    const technicianId = context.auth.uid;
    const { limit = 20 } = data;
    const snapshot = await config_1.db.collection('payouts')
...
exports.getTransactionHistory = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Auth required');
    }
    const technicianId = context.auth.uid;
    const { limit = 20, startAfter } = data;
    let query = config_1.db.collection('technician_wallets')
        .doc(technicianId)
        .collection('transactions')
        .orderBy('createdAt', 'desc')
        .limit(limit);
    if (startAfter) {
        const startDoc = await config_1.db.collection('technician_wallets')
            .doc(technicianId)
            .collection('transactions')
            .doc(startAfter)
            .get();
...
exports.generateBookingQR = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Auth required');
    }
    const technicianId = context.auth.uid;
    ...
    const existingQR = await config_1.db.collection('bookings')
        .doc(bookingId)
        .collection('payment')
        .doc('qr')
        .get();
    ...
    const qrCode = await rzp.qrCodes.create({
        type: 'upi_qr',
    ...
    await config_1.db.collection('bookings').doc(bookingId)
 ...
 */
exports.generateTechnicianWalletQR = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Auth required');
    }
    const technicianId = context.auth.uid;
    ...
    const existingQRSnapshot = await config_1.db.collection('technician_qr_codes')
        .where('technicianId', '==', technicianId)
        .where('status', '==', 'active')
    ...
    try {
        ...
        // Store QR metadata
        await config_1.db.collection('technician_qr_codes').add({
            qrId: qrCode.id,
            technicianId,
            technicianName: techData.name,
            qrImageUrl: qrCode.image_url,
...
//# sourceMappingURL=technician_withdrawal.js.map