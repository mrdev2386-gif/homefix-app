...
exports.submitPartnerApplication = void 0;
 ...
 */
exports.submitPartnerApplication = functions.region('asia-south1').https.onCall(async (data, context) => {
    try {
        // V1: Auth is in context.auth
        const auth = context.auth;
        if (!auth) {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated to submit application');
        }
        const userId = auth.uid;
        // data is already the first argument in V1 onCall
        // 0. RATE LIMITING (Harden)
        await (0, utils_1.checkRateLimit)(userId, 'submit_partner_application', 2, 1440);
        ...
        if (!data.bankDetails.accountNumber || !data.bankDetails.ifscCode || !data.bankDetails.holderName) {
            throw new functions.https.HttpsError('invalid-argument', 'Bank details must include accountNumber, ifscCode, and holderName');
        }
        // Validate arrays
        if (!Array.isArray(data.categoryIds) || data.categoryIds.length === 0) {
            throw new functions.https.HttpsError('invalid-argument', 'categoryIds must be a non-empty array');
        }
        if (!Array.isArray(data.subcategoryIds) || data.subcategoryIds.length === 0) {
            throw new functions.https.HttpsError('invalid-argument', 'subcategoryIds must be a non-empty array');
        }
        // Validate experience years
        if (typeof data.experienceYears !== 'number' || data.experienceYears <= 0) {
            throw new functions.https.HttpsError('invalid-argument', 'experienceYears must be a positive number');
        }
        ...
        if (existingApp.exists) {
            ...
            if (status === 'pending' || status === 'approved') {
                throw new functions.https.HttpsError('already-exists', 'You have already submitted an application');
            }
        }
        ...
        // Wrap other errors
        throw new functions.https.HttpsError('internal', error.message || 'Failed to submit application');
    }
});
