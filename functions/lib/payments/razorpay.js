...
exports.initiateRefund = exports.canRetryPayment = exports.handlePaymentFailure = exports.verifyPayment = exports.createPaymentOrder = exports.createRazorpayOrder = void 0;
...
const getRazorpayConfig = () => {
    ...
    if (!config.razorpay) {
        throw new functions.https.HttpsError('failed-precondition', 'Razorpay configuration not found. Run: firebase functions:config:set razorpay.key_id="xxx" razorpay.key_secret="xxx" razorpay.webhook_secret="xxx"');
    }
    const { key_id, key_secret } = config.razorpay;
    if (!key_id || !key_secret) {
        throw new functions.https.HttpsError('failed-precondition', 'Razorpay key_id and key_secret are required');
    }
 ...
 */
async function createRazorpayOrderDoc(orderId, amount, options) {
    const orderRef = config_1.db.collection('razorpayOrders').doc(orderId);
    const orderData = {
        orderId,
        amount,
...
exports.createRazorpayOrder = functions
    ...
    try {
        ...
        utils_1.logger.error('createRazorpayOrder_failed', { technicianId, amount }, error);
        throw error;
    }
}));
// ============================================================================
...
exports.createPaymentOrder = functions
    ...
    if (paymentMethod !== 'after_service') {
        throw new functions.https.HttpsError('failed-precondition', 'Invalid payment method. Only after-service payment is supported.');
    }
    // Validation 3: Booking must be in awaiting_payment state
    const currentStatus = booking.bookingStatus || booking.status;
    if (currentStatus !== 'awaiting_payment') {
        throw new functions.https.HttpsError('failed-precondition', `Payment not allowed. Booking status is "${currentStatus}". Payment is only allowed after service completion.`);
    }
    ...
    try {
        ...
        utils_1.logger.error('createPaymentOrder_failed', { bookingId, userId }, error);
        throw error;
    }
}));
// ============================================================================
...
async function handlePaymentCaptured(payload) {
    ...
    const bookingsSnapshot = await config_1.db.collection('bookings')
...
async function handlePaymentFailed(payload) {
    ...
    const bookingSnap = await config_1.db.collection('bookings')
        .where('payment.razorpayOrderId', '==', orderId)
        .limit(1)
        .get();
    if (!bookingSnap.empty) {
        const b = bookingSnap.docs[0].data();
        await (0, notifications_1.sendPushNotification)(b.customerId, 'customers', {
...
exports.verifyPayment = functions
    ...
    const currentStatus = booking.bookingStatus || booking.status;
    if (currentStatus !== 'awaiting_payment') {
        throw new functions.https.HttpsError('failed-precondition', `Cannot verify payment for booking in status: ${currentStatus}`);
    }
    ...
    try {
        ...
        await config_1.db.runTransaction(async (transaction) => {
            ...
            if (isPaid) {
                ...
                transaction.set(config_1.db.collection('payment_logs').doc(), {
                    bookingId,
                    orderId: razorpayOrderId,
                    paymentId: razorpayPaymentId,
            ...
            const orderRef = config_1.db.collection('razorpayOrders').doc(razorpayOrderId);
            ...
            if (currentBooking.technicianId) {
                const techWalletRef = config_1.db.collection('technician_wallets').doc(currentBooking.technicianId);
        ...
        utils_1.logger.error('verifyPayment_failed', { bookingId, userId }, error);
        throw error;
    }
}));
// ============================================================================
 ...
 */
exports.canRetryPayment = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    ...
    const bookingDoc = await config_1.db.collection('bookings').doc(bookingId).get();
    if (!bookingDoc.exists) {
        throw new functions.https.HttpsError('not-found', 'Booking not found');
    }
    const booking = bookingDoc.data();
...
exports.initiateRefund = functions
    ...
    try {
        ...
        try {
            ...
            if (booking.technicianId) {
                const walletRef = config_1.db.collection('technician_wallets').doc(booking.technicianId);
                ...
                if (walletDoc.exists) {
                    ...
                    if (currentBalance >= refundAmount) {
                        ...
                        await config_1.db.collection('refund_compensations').add({
                            bookingId,
                            refundId: refund.id,
                            requestId: requestId,
                            technicianId: booking.technicianId,
