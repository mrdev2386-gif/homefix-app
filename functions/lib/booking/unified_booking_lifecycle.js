...
exports.getAllTechniciansForAdmin = exports.adminChangeTechnician = exports.createBookingRequest = exports.rejectBookingByAdmin = exports.cancelBooking = exports.technicianRejectBooking = exports.completeService = exports.startService = exports.technicianAcceptBooking = exports.approveBookingByAdmin = void 0;
...
const production_hardening_1 = require("./production_hardening");
...
// ==========================================
exports.approveBookingByAdmin = functions
    .region('asia-south1')
    ...
    await db.runTransaction(async (t) => {
        ...
        (0, status_history_tracker_1.updateBookingStatus)(t, bookingRef, 'approved_by_admin', freshBooking, {
...
// ==========================================
exports.technicianAcceptBooking = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
    ...
    if (booking.technicianId !== uid) {
        throw new functions.https.HttpsError('permission-denied', 'Only the assigned technician can accept this booking');
    }
    if (booking.bookingStatus !== 'approved_by_admin') {
        throw new functions.https.HttpsError('failed-precondition', `Cannot accept booking with status: ${booking.bookingStatus}`);
    }
    await db.runTransaction(async (t) => {
        const freshDoc = await t.get(bookingRef);
        if (!freshDoc.exists)
...
// 3️⃣ START SERVICE
// ==========================================
exports.startService = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
    ...
    const booking = bookingSnap.data();
    if (booking.technicianId !== uid) {
        throw new functions.https.HttpsError('permission-denied', 'Only the assigned technician can start this service');
    }
    if (booking.bookingStatus !== 'technician_accepted') {
        throw new functions.https.HttpsError('failed-precondition', `Cannot start service with status: ${booking.bookingStatus}`);
    }
...
// ==========================================
exports.completeService = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
    ...
    const booking = bookingSnap.data();
    if (booking.technicianId !== uid) {
        throw new functions.https.HttpsError('permission-denied', 'Only the assigned technician can complete this service');
    }
    if (booking.bookingStatus !== 'service_in_progress') {
        throw new functions.https.HttpsError('failed-precondition', `Cannot complete service with status: ${booking.bookingStatus}`);
    }
    await db.runTransaction(async (t) => {
        const freshDoc = await t.get(bookingRef);
        if (!freshDoc.exists)
...
// ==========================================
exports.technicianRejectBooking = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
    ...
    const booking = bookingSnap.data();
    if (booking.technicianId !== uid) {
        throw new functions.https.HttpsError('permission-denied', 'Only the assigned technician can reject this booking');
    }
    if (booking.bookingStatus !== 'approved_by_admin') {
        throw new functions.https.HttpsError('failed-precondition', `Cannot reject booking with status: ${booking.bookingStatus}`);
    }
    await db.runTransaction(async (t) => {
        const freshDoc = await t.get(bookingRef);
        if (!freshDoc.exists)
            throw new functions.https.HttpsError('not-found', 'Booking not found');
        const freshBooking = freshDoc.data();
        (0, status_history_tracker_1.updateBookingStatus)(t, bookingRef, 'technician_rejected', freshBooking, {
            bookingStatus: 'technician_rejected',
            rejectedBy: uid,
            rejectedAt: admin.firestore.FieldValue.serverTimestamp(),
            rejectionReason: (0, security_1.sanitize)(reason) || 'Technician unavailable',
        });
    });
...
exports.cancelBooking = functions
    ...
    if (isCustomer && !isAdmin) {
        if (booking.bookingStatus !== 'pending_admin_review') {
            throw new functions.https.HttpsError('failed-precondition', 'You can only cancel a booking that is pending admin review');
        }
    }
    await db.runTransaction(async (t) => {
        const freshDoc = await t.get(bookingRef);
        ...
        (0, status_history_tracker_1.updateBookingStatus)(t, bookingRef, newStatus, freshBooking, {
            bookingStatus: newStatus,
            cancelledAt: admin.firestore.Timestamp.now(),
            cancelledBy: uid,
            cancellationReason: (0, security_1.sanitize)(reason) || 'No reason provided',
        });
    });
...
// ==========================================
exports.rejectBookingByAdmin = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
    ...
    await db.runTransaction(async (t) => {
        ...
        if (!freshDoc.exists)
            throw new functions.https.HttpsError('not-found', 'Booking not found');
        const freshBooking = freshDoc.data();
        (0, status_history_tracker_1.updateBookingStatus)(t, bookingRef, 'rejected_by_admin', freshBooking, {
            bookingStatus: 'rejected_by_admin',
            rejectedAt: admin.firestore.FieldValue.serverTimestamp(),
            rejectedBy: uid,
            rejectionReason: (0, security_1.sanitize)(reason) || 'Rejected by admin',
        });
    });
    // Notify customer
    const customerDoc = await db.collection('customers').doc(booking.customerId).get();
 ...
 */
exports.createBookingRequest = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
    ...
    try {
        ...
        try {
            const rateLimit = await (0, production_hardening_1.checkBookingRateLimit)(uid);
            if (!rateLimit.allowed) {
                throw new functions.https.HttpsError('resource-exhausted', rateLimit.error || 'Too many booking requests. Please try later.');
            }
        }
        ...
        const { serviceId, technicianId, categoryId, categoryName, scheduledDate, scheduledTime, address, subcategoryId, quantity, durationMinutes, couponCode, idempotencyKey, paymentMode, paymentMethod, price, } = data;
...
// ==========================================
exports.adminChangeTechnician = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
...
// ==========================================
exports.getAllTechniciansForAdmin = functions
    .region('asia-south1')
    .https.onCall((0, security_1.secureCallable)(async (data, context) => {
    const uid = context.auth?.uid;
    if (!uid)
 ...
 */
async function sendAdminNotification(bookingId, title) {
    try {
        ...
        for (const adminDoc of adminsSnapshot.docs) {
            ...
            if (adminData?.fcmToken) {
                notificationPromises.push((0, notification_helper_1.sendNotificationToToken)({
                    token: adminData.fcmToken,
                    title,
                    body: `New booking #${bookingId} requires approval`,
                    data: {
...
//# sourceMappingURL=unified_booking_lifecycle.js.map