...
exports.getAdminDashboard = exports.manualPayoutCheck = exports.runPayoutIntegrityCheck = exports.acknowledgeAdminAlert = exports.retryFailedWebhooks = void 0;
exports.storeFailedWebhook = storeFailedWebhook;
exports.recordBookingFailure = recordBookingFailure;
exports.recordReassignmentLoop = recordReassignmentLoop;
exports.isSystemHealthy = isSystemHealthy;
exports.triggerAdminAlert = triggerAdminAlert;
exports.validateStressTestConfig = validateStressTestConfig;
exports.checkSystemUnderStress = checkSystemUnderStress;
...
// Logger
const log = {
    info: (event, data) => console.log(`[${event}]`, JSON.stringify(data)),
    warn: (event, data) => console.warn(`[${event}]`, JSON.stringify(data)),
    error: (event, data) => console.error(`[${event}]`, JSON.stringify(data)),
};
/**
 * Store failed webhook for retry
 */
async function storeFailedWebhook(webhookId, payload, error) {
 ...
 */
async function recordBookingFailure(reason) {
 ...
 */
async function recordReassignmentLoop(bookingId) {
    const statusRef = db.collection('systemStatus').doc('health');
    await db.runTransaction(async (transaction) => {
        const doc = await transaction.get(statusRef);
        const now = admin.firestore.Timestamp.now();
 ...
 * Trigger admin alert
 */
async function triggerAdminAlert(type, metadata) {
 ...
 */
function calculateSeverity(type, metadata) {
    const criticalTypes = [
 ...
 */
function generateAlertMessage(type, metadata) {
    const messages = {
 ...
 */
async function sendAdminFCMNotification(alertId, severity, message) {
    try {
        const { sendPushNotification } = await Promise.resolve().then(() => __importStar(require('../shared/notifications')));
 ...
 */
exports.acknowledgeAdminAlert = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
 ...
 */
async function validateStressTestConfig(config) {
 ...
 */
async function checkSystemUnderStress() {
    const metrics = {};
    // Get active bookings
    ...
    // Check thresholds
    const healthy = metrics.activeBookings < 1000 &&
 ...
 */
async function verifyAppCheck(context) {
 ...
 */
function isValidAppCheckToken(token) {
    ...
    return async (data, context) => {
        if (CONFIG.requireAppCheck) {
            ...
            if (!appCheckResult.valid) {
                throw new functions.https.HttpsError('failed-precondition', `App Check required: ${appCheckResult.error}`);
            }
        }
 ...
 */
exports.manualPayoutCheck = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Auth required');
    }
    // Verify admin
    ...
    // Run check
    await (0, exports.runPayoutIntegrityCheck)(context);
    // Get latest report
 ...
 */
exports.getAdminDashboard = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Auth required');
    }
    const isAdmin = await verifyAdminRole(context.auth.uid);
...
//# sourceMappingURL=final_hardening.js.map