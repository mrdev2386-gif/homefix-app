...
import { calculateDistance, estimateEta, LatLng } from '../shared/geoutils';
 ...
 */
export async function matchAndAssignBooking(bookingId: string, options?: { forceAssign?: boolean }) {
    ...
    for (const doc of techsSnapshot.docs) {
        ...
        if (!isValidCandidate(tech, booking, config)) continue;

        // B. Geo Check
        if (!tech.geo || !tech.geo.lat || !tech.geo.lng) continue;
        const techLocation: LatLng = { lat: tech.geo.lat, lng: tech.geo.lng };
        const distance = calculateDistance(bookingLocation, techLocation);

        if (distance > config.searchRadiusKm) continue;
...
function calculateScore(tech: any, booking: any, distance: number, config: MatchingConfig) {
    ...
    const lastUpdate = tech.updatedAt ? (tech.updatedAt as admin.firestore.Timestamp).toDate().getTime() : 0;
    ...
    const bookingServices = (booking.services || []).map((s: any) => s.id);
    const techServices = tech.skills || []; // Assuming 'skills' or 'servicesOffered'
    const intersection = bookingServices.filter((s: string) => techServices.includes(s));
    let sNorm = 0.5;
    if (intersection.length === bookingServices.length) sNorm = 1.0;
    else if (intersection.length > 0) sNorm = 0.75;

    // 7. Fatigue (0..1, fresh is better)
    // Check lastJobEndedAt
    let tNorm = 1.0;
    if (tech.lastJobEndedAt) {
        const lastJobEnd = (tech.lastJobEndedAt as admin.firestore.Timestamp).toDate().getTime();
        const hoursSinceJob = (Date.now() - lastJobEnd) / (1000 * 60 * 60);
        if (hoursSinceJob < 1) tNorm = 0.5; // Just finished
        else if (hoursSinceJob < config.fatigueThresholdHours) tNorm = 0.8;
    }
...
function isValidCandidate(tech: any, booking: any, config: MatchingConfig): boolean {
    // 1. Status Check
    if (tech.status !== 'approved') return false;

    // 2. Risk Check
    // If tech doc has risk status, use it. Else assume ok.
    if (tech.riskStatus === 'suspended') return false;
    // Restricted techs might be filtered or scored low. Let's filter for now.
    if (tech.riskStatus === 'restricted') return false;

    // 3. Heartbeat (Freshness)
    if (tech.lastHeartbeatAt) {
        const hb = (tech.lastHeartbeatAt as admin.firestore.Timestamp).toDate().getTime();
        const mins = (Date.now() - hb) / (1000 * 60);
        if (mins > config.heartbeatExpiryMinutes) return false;
    }
...
async function hasTechnicianRejected(bookingId: string, techId: string): Promise<boolean> {
...
async function attemptAssignmentInTransaction(bookingId: string, candidate: ScoredCandidate, config: MatchingConfig) {
    ...
    await sendPushNotification(candidate.techId, 'technicians', {
        title: 'New Job Offer! 🚀',
...
async function handleAdminEscalation(bookingId: string, reason: string) {
    await db.collection('bookings').doc(bookingId).update({
...
export const onBookingCreatedMatch = functions.firestore
...
export const handleAssignmentResponse = functions.region('asia-south1').https.onCall(async (data, context) => {
    ...
    await db.runTransaction(async (t) => {
        ...
        if (!doc.exists) throw new functions.https.HttpsError('not-found', 'Assignment request not found');
        const req = doc.data()!;
        ...
        // Check expiry
        const expiresAt = (req.expiresAt as admin.firestore.Timestamp).toDate();
