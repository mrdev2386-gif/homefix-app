...
function logWithdrawalEvent(
  ...
  if (['request_created', 'approval_success', 'approval_failed', 'payout_failed'].includes(event)) {
    db.collection('withdrawal_logs').add({
      ...logEntry,
...
function generateRequestIdempotencyKey(technicianId: string, amount: number, timestamp: number): string {
 ...
 */
export const createWithdrawalRequest = functions.region('asia-south1').https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (todayRequests.size >= DAILY_REQUEST_LIMIT) {
            ...
            throw new functions.https.HttpsError(
                'resource-exhausted',
                `Maximum ${DAILY_REQUEST_LIMIT} withdrawal requests per day allowed`
            );
        }

        // Check for pending requests
        const pendingRequests = await db.collection('withdrawal_requests')
            .where('technicianId', '==', technicianId)
        ...
        // Update analytics
        await db.collection('system_analytics').doc('withdrawals').set({
 ...
 */
export const approveWithdrawalRequest = functions.region('asia-south1').https.onCall(async (data, context) => {
    const startTime = Date.now();
    ...
    try {
        ...
        for (let attempt = 0; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
            try {
                razorpayPayout = await (rzp as any).payouts.create({
 ...
 */
export const rejectWithdrawalRequest = functions.region('asia-south1').https.onCall(async (data, context) => {
    const startTime = Date.now();
 ...
 */
export const getMyWithdrawalRequests = functions.region('asia-south1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Auth required');
    }
