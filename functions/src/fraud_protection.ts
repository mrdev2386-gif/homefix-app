...
import { updateRiskProfile, RISK_INCREMENTS } from './shared/risk_engine';
 ...
 */
export const onBookingStatusUpdateRiskCheck = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        if (after.status === 'cancelled' && after.cancelledBy === 'system' && after.assignedTechnicianId) {
            ...
            await updateRiskProfile(after.assignedTechnicianId, 'technician', RISK_INCREMENTS.MEDIUM, 'no_show_suspected');
        }

        // Detect Service Completed Unusually Fast
        if (before.status !== 'completed' && after.status === 'completed') {
            const startTime = (after.startedAt as admin.firestore.Timestamp)?.toDate();
            const endTime = (after.completedAt as admin.firestore.Timestamp)?.toDate();
            if (startTime && endTime) {
                const durationMinutes = (endTime.getTime() - startTime.getTime()) / (1000 * 60);
                if (durationMinutes < 5) { // Threshold for fake completion
                    await updateRiskProfile(after.assignedTechnicianId, 'technician', RISK_INCREMENTS.MEDIUM, 'fake_completion_suspected', { durationMinutes });
                }
            }
        }
    });
 ...
 */
export const onReviewRiskCheck = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        ...
        if (rating <= 2 && (!reviewText || reviewText.length < 5)) {
            ...
            if (lowRatings.length >= 3) {
                await updateRiskProfile(customerId, 'customer', RISK_INCREMENTS.MEDIUM, 'rating_abuse', { count: lowRatings.length });
            }
        }
    });
 ...
 */
export const onPaymentStatusRiskCheck = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        ...
        if (payment?.status === 'failed') {
            ...
            if (recentFailures.size >= 3) {
                await updateRiskProfile(userId, 'customer', RISK_INCREMENTS.MEDIUM, 'payment_abuse');
            }
        }
    });
...
async function evaluateCustomerCancellationRisk(customerId: string) {
    ...
    if (cancellations.length >= 3 || (totalCount >= 5 && cancellationRate > 0.5)) {
        await updateRiskProfile(customerId, 'customer', RISK_INCREMENTS.MEDIUM, 'high_cancellation', { rate: cancellationRate, count: cancellations.length });
    }
}

async function evaluateCustomerRefundRisk(customerId: string) {
    ...
    if (recentRefunds.size >= 3) {
        await updateRiskProfile(customerId, 'customer', RISK_INCREMENTS.MEDIUM, 'refund_gaming');
    }
}
 ...
 */
export const onTechnicianProfileUpdateRiskCheck = functions.firestore
...
export async function evaluateTechnicianRejectionRisk(techId: string) {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    ...
    if (recentRejections.size >= 5) {
        await updateRiskProfile(techId, 'technician', RISK_INCREMENTS.MINOR, 'frequent_rejections', { count: recentRejections.size });
    }
}
